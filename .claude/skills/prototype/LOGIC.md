...
Pick whichever shape best fits the question being asked, *not* whichever is easiest to wire to a TUI. Keep it pure: no I/O, no terminal code, no `Console.WriteLine` (or your language's equivalent) for control flow. The TUI imports it and calls into it; nothing flows the other direction.
...
Add a script to the project's existing task runner — a `dotnet run --project prototypes/<name>` console app, `package.json` scripts, `Makefile`, `justfile`, `pyproject.toml`, whatever the host project already uses. The user should run `dotnet run --project <prototype-path>` (or the JS/Python/etc. equivalent) — never need to remember a raw path.
